...
// Set Mapbox token - you'll need to get your own token from https://mapbox.com
...
const RouteMap: React.FC<RouteMapProps> = ({
  ...
  useEffect(() => {
    ...
    if (!tripResult) {
      setDebugInfo('No trip result provided')
    ...
    if (!segments || segments.length === 0) {
      setDebugInfo('No route segments found in trip result')
      setError('No route segments available')
      setIsLoading(false)
      return
    }

    setDebugInfo(`Found ${segments.length} segment(s)`)
  }, [tripResult])
    ...
    const initializeMap = async () => {
      try {
        ...
        map.current.on('load', async () => {
          ...
          console.log('Adding markers for:', { startLoc, endLoc })
          ...
          const startMarker = new mapboxgl.Marker({
            ...
            .setPopup(
              new mapboxgl.Popup({ offset: 25 })
                .setHTML(`
                  <div class="p-3">
                    <h3 class="font-semibold text-green-600 mb-1">Pickup Location</h3>
          ...
          if (geometry) {
            ...
            console.warn('Could not fetch route geometry from Mapbox')
          }
    ...
    return (
      <div className={`${height} bg-gray-50 rounded-lg flex items-center justify-center border border-gray-200 ${className}`}>
        <div className="text-center p-6 max-w-md">
          ...
          {debugInfo && (
            <p className="text-xs text-gray-500 mb-3">Debug: {debugInfo}</p>
          )}
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      {mapLoaded && !isLoading && firstSegment && (
